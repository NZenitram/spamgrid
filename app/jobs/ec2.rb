...
class Ec2
  ...
  def ubuntu_update
    Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file ) do |ssh|
      ...
      ssh.exec "sudo apt-get install -y curl gnupg build-essential"
      ssh.loop
    end
  end

  def install_ruby_and_node
    ...
    Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file ) do |ssh|
      ...
      ssh.exec "sudo curl -sSL https://get.rvm.io | sudo bash -s stable"
      ssh.loop
    end
    ...
    Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file ) do |ssh|
      ...
      ssh.exec "rvm install ruby"
      ssh.exec "rvm --default use ruby"
      ssh.exec "gem install bundler --no-rdoc --no-ri"
      ssh.exec "sudo apt-get install -y nodejs && sudo ln -sf /usr/bin/nodejs /usr/local/bin/node"
      ssh.loop
    end
  end
    #
    ...
    # end
    #
    # # copy and paste local .ssh keys for git clone
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file ) do |ssh|
    ...
    # end
    #
    # #select ruby version and install git
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    ...
    # end
    #
    # #install requisite ruby-dev package
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    #   ssh.exec "sudo apt-get install -y ruby-dev"
    #   ssh.loop
    # end
    #
    # #install requisite libpq-dev package
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    ...
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    #   ssh.exec "sudo apt-get install -y build-essential patch"
    #   ssh.loop
    # end
    #
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    #   ssh.exec "sudo apt-get install -y ruby-dev zlib1g-dev liblzma-dev"
    #   ssh.loop
    # end
    #
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    ...
    #
    # Net::SSH.start(instance.public_dns_name, ssh_username, :keys => private_key_file) do |ssh|
    #   ssh.exec "cd /etc/nginx && sudo wget -q https://s3-us-west-1.amazonaws.com/myconfigfiles/nginx.conf -O /etc/nginx/nginx.conf"
    #   ssh.exec "cd /etc/nginx/sites-enabled && sudo wget -q https://s3-us-west-1.amazonaws.com/myconfigfiles/spamgrid.conf"
    #   ssh.loop
    # end
